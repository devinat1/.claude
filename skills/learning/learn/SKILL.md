...
- **lab** → `lab-creator` is supplied by `devinat1/engineering-skills`. If it is unavailable, report `npx skills@latest add devinat1/engineering-skills` and do not scaffold. Otherwise, say that you are using `/lab-creator` to turn the confirmed gap into a hands-on exercise, then invoke it with the concept + gap context (the `confirmed gap`, the `domain`, code-vs-conceptual). It scaffolds the files and returns the path + visible cases. **Generate at most ONE lab per run** — if several gaps chose `lab`, build it for the highest-leverage one and `log` the rest. Do not narrate scaffolding beyond that one disclosure — save the path for Phase 6.

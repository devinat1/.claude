...
description: Conducts realistic system design interview practice sessions using the Socratic method. Claude plays the role of a tough but fair interviewer, guiding the user through a structured framework (Requirements → Core Entities → API Design → Data Flow → High-Level Design → Capacity Planning → Deep Dives) with maximum probing depth. Use this skill whenever the user wants to practice a system design interview, asks to be interviewed on a system design topic, mentions "system design practice," "mock system design interview," or wants feedback on how they'd design a system. Also trigger when the user says things like "interview me on designing X," "let's do a system design for X," or "practice system design." Do NOT use for actual system design work — this is specifically for interview practice with Socratic questioning, real-time phase feedback, and a final scorecard.
---

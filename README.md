...
An arrow means the source suggests the target after an intentional successful
completion; it does not invoke the target automatically.
The behavior contract and maintained map live in
[skill connections](./docs/skill-connections.md).

```mermaid
flowchart LR
  clarify["/clarify"] --> coherent["/coherent"]
  clarify --> unscramble["/unscramble"]
  clarify --> pragmatic["/pragmatic"]
  clarify --> mentor["/mentor"]
  unscramble --> confounder["/confounder"]
  unscramble --> ramble["/ramble"]
  blog["/blog"] --> post["/post"]
  blog --> youtube["/youtube"]
  todo_triage["/todo-triage"] --> focus["/focus"]
  apset["/apset"] --> research_gap["/research-gap"]
  apset --> research_advisor["/research-advisor"]
  research_advisor --> dunning_krueger["/dunning-krueger"]
  onboard["/onboard"] --> scale["/scale"]
  system["/system"] --> dunning_krueger
  interviewer["/interviewer"] --> dunning_krueger
  scale --> break_it["/break-it"]
  scale --> learn["/learn"]
  momtest["/momtest"] --> ramble
  momtest --> linear["/linear"]
  pragmatic --> linear
```
